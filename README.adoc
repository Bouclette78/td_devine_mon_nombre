...
+
....
on donne pas les droits d'accès a la variable de manière permanente (emprunt de la valeur mais pas propriété)
....
.. Quel est le type de retour de `read_line` ? Quel est son utilité ?
+
....
c'est un booléen qui dit si c'est vide ou pas 
....
...
+
....
repertoir src avec main et cargo.toml
....
